...
public class SoundFilterEditScreen extends Screen {
    ...
    public SoundFilterEditScreen(Screen parent, String entityId, String entityName) {
        super(new TextComponent("Sound Filters: " + entityName));
        this.parent = parent;
    ...
    protected void init() {
        ...
        searchBox = new EditBox(this.font, centerX - FIELD_WIDTH / 2, startY, FIELD_WIDTH - 110, FIELD_HEIGHT,
                new TextComponent("Search Sounds"));
        searchBox.setResponder(this::onSearchChanged);
        searchBox.setMaxLength(100);
        this.addRenderableWidget(searchBox);

        toggleFilterButton = new Button(centerX + FIELD_WIDTH / 2 - 100, startY, 100, FIELD_HEIGHT,
                new TextComponent("Show Blocked"), b -> toggleShowBlocked());
        this.addRenderableWidget(toggleFilterButton);

        int buttonY = this.height - 30;

        clearButton = new Button(centerX - 210, buttonY, 100, 20, new TextComponent("Clear All"), b -> clearFilters());
        this.addRenderableWidget(clearButton);

        saveButton = new Button(centerX - 100, buttonY, 100, 20, new TextComponent("Save"), b -> saveFilters());
        this.addRenderableWidget(saveButton);

        backButton = new Button(centerX + 10, buttonY, 100, 20, new TextComponent("Cancel"),
    ...
    private void toggleShowBlocked() {
        ...
        scrollOffset = 0;
        toggleFilterButton.setMessage(new TextComponent(showOnlyBlocked ? "Show All" : "Show Blocked"));
    }
    ...
    private void renderSuggestionTooltip(PoseStack poseStack, int mouseX, int mouseY) {
        ...
        if (mouseX >= startX && mouseX <= startX + contentWidth &&
            ...
            if (index >= 0 && index < filtered.size()) {
                ...
                tooltip.add(new TextComponent("§6" + entry.category));

                if (blockedSounds.contains(entry.soundId)) {
                    tooltip.add(new TextComponent("§cClick to unblock"));
                } else {
                    tooltip.add(new TextComponent("§aClick to block"));
                }
