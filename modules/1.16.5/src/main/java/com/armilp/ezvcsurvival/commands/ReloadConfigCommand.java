...
public class ReloadConfigCommand {
    ...
    private static int executeReload(CommandContext<CommandSource> context) {
        ...
        try {
            source.sendSuccess(new StringTextComponent("§6[EZVCSurvival] §eReloading configuration files..."), false);

            // Entity Voice Config
            try {
                reloadEntityVoiceConfig();
            } catch (Exception e) {
                source.sendFailure(new StringTextComponent("§6[EZVCSurvival] §cFailed to reload EntityVoiceConfig: " + e.getMessage()));
            ...
            try {
                ...
                source.sendFailure(new StringTextComponent("§6[EZVCSurvival] §cFailed to reload GeneralSoundConfig: " + e.getMessage()));
                if (VoiceConfig.DEBUG.get()) {
                    e.printStackTrace();
                }
            }
            ...
            try {
                ...
                source.sendFailure(new StringTextComponent("§6[EZVCSurvival] §cFailed to reload GunfireConfig: " + e.getMessage()));
                if (VoiceConfig.DEBUG.get()) {
                    e.printStackTrace();
                }
            }
            ...
            try {
                ...
                source.sendFailure(new StringTextComponent("§6[EZVCSurvival] §cFailed to reload SoundConfig: " + e.getMessage()));
                if (VoiceConfig.DEBUG.get()) {
                    e.printStackTrace();
                }
            }
            ...
            try {
                ...
                source.sendFailure(new StringTextComponent("§6[EZVCSurvival] §cFailed to refresh mob goals: " + e.getMessage()));
                if (VoiceConfig.DEBUG.get()) {
                    e.printStackTrace();
                }
            }

            source.sendSuccess(new StringTextComponent("§6[EZVCSurvival] §aAll configurations reloaded successfully!"), false);
            return 1;

        } catch (Exception e) {
            source.sendFailure(new StringTextComponent("§6[EZVCSurvival] §cAn unexpected error occurred: " + e.getMessage()));
            if (VoiceConfig.DEBUG.get()) {
                e.printStackTrace();
            }
            return 0;
    ...
    private static void refreshAllMobGoals(CommandSource source) {
        if (source.getServer() != null) {
            int totalRefreshed = 0;
            for (ServerWorld level : source.getServer().getAllLevels()) {
                for (net.minecraft.entity.Entity entity : level.getAllEntities()) {
                    if (entity instanceof MobEntity && entity instanceof IGoalRefresher) {
