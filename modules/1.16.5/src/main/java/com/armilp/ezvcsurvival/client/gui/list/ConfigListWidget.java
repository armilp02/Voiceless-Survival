...
public class ConfigListWidget extends ExtendedList<ConfigListWidget.Entry> {
    ...
    public class UniversalEntry extends Entry {
        ...
        public void render(MatrixStack matrixStack, int index, int top, int left, int width, int height,
            ...
            if (validHover) {
                ...
                fill(matrixStack, actualLeft, top, actualLeft + actualWidth, top + height, 0x30FFFFFF);
                fill(matrixStack, actualLeft, top, actualLeft + actualWidth, top + 1, 0x60FFFFFF);
                fill(matrixStack, actualLeft, top + height - 1, actualLeft + actualWidth, top + height, 0x60FFFFFF);
            }

            // Alternar color de fondo para mejorar la legibilidad
            if (index % 2 == 0) {
                fill(matrixStack, actualLeft, top, actualLeft + actualWidth, top + height, 0x10000000);
            }
            ...
            if (item instanceof ConfigListScreen.EntityConfigItem) {
                renderEntityConfig(matrixStack, textLeft, top, actualWidth, height, centerY, statusRight);
            } else if (item instanceof ConfigListScreen.SoundConfigItem) {
                renderSoundConfig(matrixStack, textLeft, top, actualWidth, height, centerY, statusRight);
            } else if (item instanceof ConfigListScreen.EntityReactionItem) {
                renderEntityReaction(matrixStack, textLeft, top, actualWidth, height, centerY, statusRight);
            } else if (item instanceof ConfigListScreen.GunfireEntityItem) {
                renderGunfireEntity(matrixStack, textLeft, top, actualWidth, height, centerY, statusRight);
            }
        }

        private void renderEntityConfig(MatrixStack matrixStack, int textLeft, int top, int width, int height,
            ...
            Minecraft.getInstance().font.draw(matrixStack, displayName, textLeft, centerY - 4, 0xFFFFFF);

            ITextComponent statusText = entityItem.getConfig().enabled
                    ? new TranslationTextComponent("gui.ezvcsurvival.enabled")
                    : new TranslationTextComponent("gui.ezvcsurvival.disabled");
            ...
            fill(matrixStack, statusRight - statusWidth - 6, centerY - 8, statusRight, centerY + 8,
                    entityItem.getConfig().enabled ? 0x2055FF55 : 0x20FF5555);

            Minecraft.getInstance().font.draw(matrixStack, statusText,
                    statusRight - statusWidth - 3, centerY - 4, statusColor);
        }

        private void renderSoundConfig(MatrixStack matrixStack, int textLeft, int top, int width, int height,
                                       int centerY, int statusRight) {
            ...
            ITextComponent statusText = soundItem.getConfig().enabled
                    ? new TranslationTextComponent("gui.ezvcsurvival.enabled")
                    : new TranslationTextComponent("gui.ezvcsurvival.disabled");
        ...
        private void renderEntityReaction(MatrixStack matrixStack, int textLeft, int top, int width, int height,
                                          int centerY, int statusRight) {
            ...
            ITextComponent statusText = entityItem.reaction.enabled
                    ? new TranslationTextComponent("gui.ezvcsurvival.enabled")
                    : new TranslationTextComponent("gui.ezvcsurvival.disabled");
        ...
        private void renderGunfireEntity(MatrixStack matrixStack, int textLeft, int top, int width, int height,
                                         int centerY, int statusRight) {
        ...
        private String getEntityDisplayName(String entityId) {
            try {
                ...
                if (ForgeRegistries.ENTITIES.containsKey(key)) {
                    EntityType<?> entityType = ForgeRegistries.ENTITIES.getValue(key);
                    if (entityType != null) {
        ...
        public void renderTooltip(MatrixStack matrixStack, int mouseX, int mouseY) {
            List<ITextComponent> tooltipLines = getTooltipLines();
            if (tooltipLines.isEmpty()) {
                return;
            }

            net.minecraft.client.gui.FontRenderer font = Minecraft.getInstance().font;
            net.minecraft.client.gui.screen.Screen screen = Minecraft.getInstance().screen;
            ...
            int maxLineWidth = 0;
            for (ITextComponent c : tooltipLines) {
                int w = font.width(c);
                if (w > maxLineWidth) {
                    maxLineWidth = w;
                }
            ...
            fill(matrixStack, tooltipX - 3, tooltipY - 4, tooltipX + maxLineWidth + padding, tooltipY + tooltipHeight, 0xF0100010);
            fill(matrixStack, tooltipX - 4, tooltipY - 5, tooltipX + maxLineWidth + padding + 1, tooltipY - 4, 0x505000FF);
            fill(matrixStack, tooltipX - 4, tooltipY + tooltipHeight, tooltipX + maxLineWidth + padding + 1, tooltipY + tooltipHeight + 1, 0x505000FF);
            fill(matrixStack, tooltipX - 4, tooltipY - 4, tooltipX - 3, tooltipY + tooltipHeight, 0x505000FF);
            fill(matrixStack, tooltipX + maxLineWidth + padding, tooltipY - 4, tooltipX + maxLineWidth + padding + 1, tooltipY + tooltipHeight, 0x505000FF);

            // Draw tooltip text
            for (int i = 0; i < tooltipLines.size(); i++) {
                font.draw(matrixStack, tooltipLines.get(i), tooltipX, tooltipY + i * lineHeight, 0xFFFFFF);
            }
        }

        private List<ITextComponent> getTooltipLines() {
            ...
            if (item instanceof ConfigListScreen.EntityConfigItem) {
                ...
                tooltip.add(new StringTextComponent("§6§l" + entityItem.getDisplayName()));
                tooltip.add(new StringTextComponent("§7ID: §f" + entityItem.getId()));
                tooltip.add(new StringTextComponent(""));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.configuration"));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.enabled",
                        config.enabled ? new TranslationTextComponent("gui.ezvcsurvival.enabled") : new TranslationTextComponent("gui.ezvcsurvival.disabled")));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.speed", new StringTextComponent("§e" + config.speed)));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.range", new StringTextComponent("§e" + config.range)));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.threshold", new StringTextComponent("§e" + config.threshold)));

            } else if (item instanceof ConfigListScreen.SoundConfigItem) {
                ConfigListScreen.SoundConfigItem soundItem = (ConfigListScreen.SoundConfigItem) item;
                com.armilp.ezvcsurvival.config.GeneralSoundsConfig.SoundEntry config = soundItem.getConfig();

                tooltip.add(new StringTextComponent("§6§lSound: §f" + soundItem.getId()));
                tooltip.add(new StringTextComponent(""));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.configuration"));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.enabled",
                        config.enabled ? new TranslationTextComponent("gui.ezvcsurvival.enabled") : new TranslationTextComponent("gui.ezvcsurvival.disabled")));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.speed_multiplier", new StringTextComponent("§e" + config.speed_multiplier)));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.range_multiplier", new StringTextComponent("§e" + config.range_multiplier)));
                ...
                tooltip.add(new StringTextComponent("§6§l" + entityName));
                tooltip.add(new StringTextComponent("§7ID: §f" + entityItem.id));
                tooltip.add(new StringTextComponent(""));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.sound_reaction"));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.enabled",
                        reaction.enabled ? new TranslationTextComponent("gui.ezvcsurvival.enabled") : new TranslationTextComponent("gui.ezvcsurvival.disabled")));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.speed", new StringTextComponent("§e" + reaction.speed)));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.range", new StringTextComponent("§e" + reaction.range)));

            } else if (item instanceof ConfigListScreen.GunfireEntityItem) {
                ConfigListScreen.GunfireEntityItem entityItem = (ConfigListScreen.GunfireEntityItem) item;
                com.armilp.ezvcsurvival.config.GunfireConfig.Reaction reaction = entityItem.reaction;
                String entityName = getEntityDisplayName(entityItem.id);

                tooltip.add(new StringTextComponent("§6§l" + entityName));
                tooltip.add(new StringTextComponent("§7ID: §f" + entityItem.id));
                tooltip.add(new StringTextComponent(""));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.gunfire_reaction"));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.enabled",
                        reaction.enabled ? new TranslationTextComponent("gui.ezvcsurvival.enabled") : new TranslationTextComponent("gui.ezvcsurvival.disabled")));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.speed", new StringTextComponent("§e" + reaction.speed)));
                tooltip.add(new TranslationTextComponent("tooltip.ezvcsurvival.range", new StringTextComponent("§e" + reaction.range)));
        ...
        private void openEditScreen() {
            ...
            if (item instanceof ConfigListScreen.EntityConfigItem) {
                ...
                elementName = getEntityDisplayName(((ConfigListScreen.GunfireEntityItem) item).id);
            }
        ...
        private String getElementId() {
            if (item instanceof ConfigListScreen.EntityConfigItem) {
                ...
                return ((ConfigListScreen.GunfireEntityItem) item).id;
            }
            return "";
        }
    }
