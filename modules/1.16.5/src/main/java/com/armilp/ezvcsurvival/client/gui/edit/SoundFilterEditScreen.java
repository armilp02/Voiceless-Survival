...
public class SoundFilterEditScreen extends Screen {
    ...
    public SoundFilterEditScreen(Screen parent, String entityId, String elementName) {
        super(new StringTextComponent("Sound Filters: " + elementName));
        this.parent = parent;
    ...
    private void loadSuggestions() {
        ...
        for (Object soundObj : ForgeRegistries.SOUND_EVENTS) {
            net.minecraft.util.SoundEvent sound = (net.minecraft.util.SoundEvent) soundObj;
    ...
    protected void init() {
        ...
        searchBox = new TextFieldWidget(this.font, centerX - FIELD_WIDTH / 2, startY, FIELD_WIDTH - 110, FIELD_HEIGHT,
                new StringTextComponent("Search Sounds"));
        searchBox.setResponder(new java.util.function.Consumer<String>() {
            @Override
        ...
        toggleFilterButton = this.addButton(new Button(centerX + FIELD_WIDTH / 2 - 100, startY, 100, FIELD_HEIGHT,
                new StringTextComponent("Show Blocked"), b -> toggleShowBlocked()));

        int buttonY = this.height - 30;

        clearButton = this.addButton(new Button(centerX - 210, buttonY, 100, 20, new StringTextComponent("Clear All"), b -> clearFilters()));

        saveButton = this.addButton(new Button(centerX - 100, buttonY, 100, 20, new StringTextComponent("Save"), b -> saveFilters()));

        backButton = this.addButton(new Button(centerX + 10, buttonY, 100, 20, new StringTextComponent("Cancel"),
    ...
    private void toggleShowBlocked() {
        ...
        scrollOffset = 0;
        toggleFilterButton.setMessage(new StringTextComponent(showOnlyBlocked ? "Show All" : "Show Blocked"));
    }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTick) {
        ...
        this.font.draw(matrixStack, "Search and click to block/unblock sounds:",
                this.width / 2 - FIELD_WIDTH / 2, 25, 0xFFFFFF);

        renderSuggestions(matrixStack, mouseX, mouseY);

        drawCenteredString(matrixStack, this.font, "§7Blocked: " + blockedSounds.size() + " sounds",
        ...
        renderSuggestionTooltip(matrixStack, mouseX, mouseY);
    }

    private void renderSuggestions(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        fill(matrixStack, startX, startY, startX + FIELD_WIDTH, endY, 0xDD000000);
        fill(matrixStack, startX, startY, startX + FIELD_WIDTH, startY + 1, 0xFF555555);
        fill(matrixStack, startX, endY - 1, startX + FIELD_WIDTH, endY, 0xFF555555);

        if (filtered.isEmpty()) {
            String noResults = query.isEmpty() ? "Start typing to search..." : "No sounds found";
            drawCenteredString(matrixStack, this.font, noResults,
            ...
            for (int i = scrollOffset; i < filtered.size() && visibleCount < maxVisible; i++) {
                ...
                if (i % 2 == 0) {
                    fill(matrixStack, startX, renderY, startX + contentWidth, renderY + SUGGESTION_HEIGHT, 0x20FFFFFF);
                }
                if (isHovered) {
                    fill(matrixStack, startX, renderY, startX + contentWidth, renderY + SUGGESTION_HEIGHT, 0x40FFFFFF);
                }

                if (!entry.category.equals(lastCategory)) {
                    fill(matrixStack, startX, renderY, startX + 4, renderY + SUGGESTION_HEIGHT, getCategoryColor(entry.category));
                ...
                this.font.draw(matrixStack, displayText, startX + 10, renderY + 6, textColor);

                if (isBlocked) {
                    fill(matrixStack, startX + contentWidth - 25, renderY + 5, startX + contentWidth - 5, renderY + SUGGESTION_HEIGHT - 5, 0xFF555555);
                    this.font.draw(matrixStack, "✓", startX + contentWidth - 20, renderY + 6, 0xFFFFFF);
                }
            ...
            if (filtered.size() > maxVisible) {
                ...
                fill(matrixStack, scrollbarX, startY, scrollbarX + SCROLLBAR_WIDTH, endY, 0xFF222222);
                ...
                fill(matrixStack, scrollbarX + 1, scrollbarThumbY, scrollbarX + SCROLLBAR_WIDTH - 1,
                        scrollbarThumbY + scrollbarThumbHeight, thumbColor);
            }
        }
    }

    private void renderSuggestionTooltip(MatrixStack matrixStack, int mouseX, int mouseY) {
        String query = searchBox.getValue().toLowerCase().trim();
        List<SuggestionEntry> filtered = new ArrayList<SuggestionEntry>();
        for (SuggestionEntry s : suggestions) {
            if (showOnlyBlocked && !blockedSounds.contains(s.soundId)) {
        ...
        if (mouseX >= startX && mouseX <= startX + contentWidth &&
            ...
            if (index >= 0 && index < filtered.size()) {
                ...
                tooltip.add(new StringTextComponent("§6" + entry.category));

                if (blockedSounds.contains(entry.soundId)) {
                    tooltip.add(new StringTextComponent("§cClick to unblock"));
                } else {
                    tooltip.add(new StringTextComponent("§aClick to block"));
                }
    ...
    private String truncateText(String text, int maxWidth) {
        ...
        for (char c : text.toCharArray()) {
            truncated.append(c);
            if (this.font.width(truncated.toString() + ellipsis) >= maxWidth) {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (isDraggingScrollbar) {
            ...
            int scrollbarTrackHeight = boxHeight;
            int scrollbarThumbHeight = Math.max(30, (maxVisible * scrollbarTrackHeight) / filteredCount);
